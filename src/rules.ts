...
/** 快照（HTTP / 工具共用）。 */
export function snapshot(state: AchieveState, unlocked: Record<string, number>, installed: Set<string> = new Set()): Record<string, unknown> {
  return {
    total: ACHIEVEMENTS.length,
    unlocked: Object.keys(unlocked).length,
    achievements: ACHIEVEMENTS.map(achievement => viewOf(
      achievement,
      state,
      unlocked,
      achievement.plugin === null || installed.has(achievement.plugin),
    )),
  }
}
